...
pub fn token_überprüfen<A: Addr>(knoten_id: U160, addr: &A, token: &[u8]) -> bool {
...
fn token_generieren_intern<A: Addr>(knoten_id: U160, addr: &A, letztes_zeitfenster: bool) -> [u8;8] {
	let mut hasher = DefaultHasher::default();
	hasher.write(geheim.as_slice());
	knoten_id.hash(&mut hasher);
	addr.hash(&mut hasher);
	let mut zeitfenster = std::time::UNIX_EPOCH.elapsed().unwrap().as_secs() / TOKEN_ZEITFENSTER_SEK;
	if letztes_zeitfenster { zeitfenster = zeitfenster.saturating_sub(1); }
