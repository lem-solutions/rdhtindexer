...
impl<T: PartialEq> PeerTabelle<T> {
	...
	pub fn bep51_anfrage(&mut self) -> &Vec<U160> {
		if self.bep51_auswahl.len() < self.bep51_auswahl_größe
			|| self.bep51_auswahl_zeitstempel.elapsed() > self.bep51_interval
		{
			self.alte_hashes_entfernen();
			self.bep51_auswahl = self
				.peers
				.keys()
				.cloned()
				.choose_multiple(&mut rand::rng(), self.bep51_auswahl_größe);
		}
		&self.bep51_auswahl
	}
	...
	pub fn peer_einfügen(&mut self, info_hash: U160, peer: T) {
		...
		while v
			.front()
			.map(|(zeit, _)| zeit.elapsed() > self.max_alter)
			.unwrap_or(false)
		{
			v.pop_front();
		}
	...
	fn alte_hashes_entfernen(&mut self) {
		while let Some(e) = self.info_hash_alter.first_entry() {
			if e.key().elapsed() <= self.max_alter {
				break;
			}
