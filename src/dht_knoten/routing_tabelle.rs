...
impl<T> KnotenInfo<T> {
	fn status(&self) -> KnotenStatus {
		if self.fehleranzahl > MAX_FEHLER { return KnotenStatus::Schlecht; }
		if self.hat_geantwortet && self.letzte_nachicht.map(|i| i.elapsed() <= ZEITFENSTER).unwrap_or(false) && self.fehleranzahl.0 == 0 {
...
impl<T> Bucket<T> {
	fn knoten_mit_id_mut<F: Fn(&mut KnotenInfo<T>)>(&mut self, id: U160, f: &F) -> bool {
		for knotenslot in self.knoten.iter_mut() {
			if knotenslot.is_none() { continue; }
	...
	fn knoten_einfügen<F: Fn(&mut KnotenInfo<T>)>(&mut self, id: U160, addr: T, fragwürdig_ersetzen: bool, f: &F) -> Option<T> {
		for slot in self.knoten.iter_mut() {
			if slot.as_ref().map(|k| k.status() == KnotenStatus::Schlecht).unwrap_or(true) {
		...
		if !fragwürdig_ersetzen { return Some(addr); }
...
impl<T> RoutingTabelle<T> {
	...
	pub fn knoten_extrahieren(self) -> impl Iterator<Item=KnotenInfo<T>> {
		self.buckets
			...
			.flatten()
			.filter_map(|k| k)
			.filter(|k| k.status() == KnotenStatus::Gut || k.status() == KnotenStatus::Fragwürdig)
	}
	...
	pub fn nächste_k_knoten<'a>(&'a self, ziel: U160, puffer: &mut noalloc_vec_rs::vec::Vec<(U160, &'a T), K>)  {
		puffer.clear();
		...
		for knoten in self.iter_gute_knoten() {
			...
			for (i, (k_id, _k_addr)) in puffer.iter().enumerate() {
				idx = i;
				if *k_id ^ ziel < knoten.id ^ ziel { break; }
			};
			if idx < K {
				if puffer.len() == K {
	...
	fn bucket_für_id(&self, id: U160) -> &Bucket<T> {
		...
		&self.buckets[self.eigene_id.erstes_ungleiches_bit(id).min(self.buckets.len()-1)]
	}
	...
	fn bucket_aufteilen(&mut self) {
		...
		for knoten_slot in bestehender_bucket.knoten.iter_mut() {
			...
			if verschieben {
				...
				*neuer_bucket.knoten.iter_mut().find(|x| x.is_none()).unwrap() = knoten_slot.take()
			}
		}
	...
	fn knoten_finden_oder_einfügen<F: Fn(&mut KnotenInfo<T>)>(&mut self, id: U160, addr: T, fragwürdig_ersetzen: bool, f: &F) -> bool {
		let bucket = self.bucket_für_id_mut(id);
		if bucket.knoten_mit_id_mut(id, f) { return true; }
		let mut addr = if let Some(a) = bucket.knoten_einfügen(id, addr, fragwürdig_ersetzen, f) { a } else { return true; };
		
		let max_bucket = self.eigene_id.erstes_ungleiches_bit(id);
		while max_bucket >= self.buckets.len() {
			self.bucket_aufteilen();
			addr = if let Some(a) = self.buckets.last_mut().unwrap().knoten_einfügen(id, addr, fragwürdig_ersetzen, f) { a } else { return true; };
		}
		false
	}
	
	pub fn anfrage_erhalten(&mut self, id: U160, addr: T) {
		self.knoten_finden_oder_einfügen(id, addr, false, &|k| k.letzte_nachicht = Some(Instant::now()));
	}
	...
	pub fn fehlschlag(&mut self, id: U160) {
		self.bucket_für_id_mut(id).knoten_mit_id_mut(id,&|k| k.fehleranzahl += 1);
	}
	
	pub fn knoten_info_einfügen(&mut self, info: KnotenInfo<T>) {
		let fragwürdig_ersetzen = info.status() == KnotenStatus::Gut;
		self.knoten_finden_oder_einfügen(info.id, info.addr, fragwürdig_ersetzen, &|k| {
