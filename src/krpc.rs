...
impl<A: Addr> ToBencode for KrpcNachricht<A> {
	...
	fn encode(&self, en: SingleItemEncoder) -> Result<(), EnErr> {
		en.emit_unsorted_dict(|e_haupt| {
			e_haupt.emit_pair_with(b"t", |e| {
				e.emit_bytes(self.transaktionsnummer.as_slice())
			})?;
			...
			match self.inhalt {
				...
				KrpcInhalt::Antwort {
					...
					if let Some(ip) = ext_ip {
						...
						e_haupt
							.emit_pair_with(b"ip", |e| e.emit_bytes(puffer.as_slice()))?;
					}
...
impl<A: Addr> KrpcNachricht<A> {
	...
	pub fn einlesen<F: FnOnce(&[u8]) -> Option<AnfrageMethode>>(
		daten: &[u8],
		methode_für_txid: F,
	) -> Result<Self, DeErr> {
		let mut dekodierer =
			bendy::decoding::Decoder::new(daten).with_max_depth(16);
		let mut hauptobjekt = dekodierer
			.next_object()?
		...
		if fehler.is_some() && nachricht_typ != Some(NachrichtTyp::Fehler) {
			return Err(DeErr::unexpected_token(
				"Haupt-Dict enthält „e“",
				"obwohl y != e",
			));
		}
		...
		match nachricht_typ {
			Some(NachrichtTyp::Anfrage) => {
				...
				let krpc_inhalt_res =
					match anfrage_methode.ok_or(DeErr::missing_field("q"))? {
						AnfrageMethode::Ping => element_suchen(
							hauptobjekt2,
							b"a",
							KrpcAnfrage::ping_argumente_einlesen,
						),
						AnfrageMethode::FindNode => element_suchen(
							hauptobjekt2,
							b"a",
							KrpcAnfrage::find_node_argumente_einlesen,
						),
						AnfrageMethode::GetPeers => element_suchen(
							hauptobjekt2,
							b"a",
							KrpcAnfrage::get_peers_argumente_einlesen,
						),
						AnfrageMethode::AnnouncePeer => element_suchen(
							hauptobjekt2,
							b"a",
							KrpcAnfrage::announce_peer_argumente_einlesen,
						),
						AnfrageMethode::SampleInfohashes => element_suchen(
							hauptobjekt2,
							b"a",
							KrpcAnfrage::sample_infohashes_argumente_einlesen,
						),
						AnfrageMethode::Unbekannt(name) => {
							element_suchen(hauptobjekt2, b"a", |a| {
								KrpcAnfrage::unbekannte_methode_argumente_einlesen(a, name)
							})
						}
					};

				let krpc_inhalt =
					krpc_inhalt_res?.ok_or(DeErr::missing_field("a"))??;

				Ok(KrpcNachricht {
					transaktionsnummer: transaktionsnummer
						.ok_or(DeErr::missing_field("t"))?
						.to_vec(),
			...
			Some(NachrichtTyp::Antwort) => {
				...
				// Datentyp: Result<Option<Result<KrpcInhalt<A>>>>
				let krpc_inhalt_res = match methode_für_txid(
					transaktionsnummer
						.as_ref()
						.ok_or(DeErr::missing_field("t"))?
						.as_slice(),
				...
				// *würg*
				.ok_or(DeErr::unexpected_token(
					"Bekannte Tranaktionsnummer",
					"Unbekannte Tranaktionsnummer",
				))? {
					AnfrageMethode::Ping => element_suchen(hauptobjekt2, b"r", |a| {
						KrpcAntwort::ping_argumente_einlesen(a, ext_ip)
					}),
					AnfrageMethode::FindNode => element_suchen(hauptobjekt2, b"r", |a| {
						KrpcAntwort::find_node_argumente_einlesen(a, ext_ip)
					}),
					AnfrageMethode::GetPeers => element_suchen(hauptobjekt2, b"r", |a| {
						KrpcAntwort::get_peers_argumente_einlesen(a, ext_ip)
					}),
					AnfrageMethode::AnnouncePeer => {
						element_suchen(hauptobjekt2, b"r", |a| {
							KrpcAntwort::announce_peer_argumente_einlesen(a, ext_ip)
						})
					}
					AnfrageMethode::SampleInfohashes => {
						element_suchen(hauptobjekt2, b"r", |a| {
							KrpcAntwort::sample_infohashes_argumente_einlesen(a, ext_ip)
						})
					}
					// Wir können keine Anfrage mit Unbekannter Methode abgeschickt haben.
					// Das wäre auf jeden Fall ein Bug, deswegen panic.
					AnfrageMethode::Unbekannt(name) => panic!(
						"methode_für_txid hat AnfrageMethode::Unbekannt({name:?}) zurückgegeben"
					),
				};
...
impl KrpcAnfrage {
	pub fn methode(&self) -> AnfrageMethode {
		match self {
			...
			KrpcAnfrage::UnbkannteMethode { name } => {
				AnfrageMethode::Unbekannt(name.clone())
			}
		}
	}

	fn argumente_ausgeben(
		&self,
		en: &mut UnsortedDictEncoder,
	) -> Result<(), EnErr> {
		match self {
			KrpcAnfrage::Ping => {}
			...
			KrpcAnfrage::UnbkannteMethode { .. } => {
				panic!(
					"Es wurde versucht eine `KrpcAnfage` mit Unbekannter Methode zu kodieren"
				)
			}
		}
	...
	fn ping_argumente_einlesen<A: Addr>(
		obj: Object,
	) -> Result<KrpcInhalt<A>, DeErr> {
	...
	fn find_node_argumente_einlesen<A: Addr>(
		obj: Object,
	) -> Result<KrpcInhalt<A>, DeErr> {
		let mut d = obj.try_into_dictionary()?;
		dict_einlesen!(&mut d,
		...
		Ok(KrpcInhalt::Anfrage {
			id: id.ok_or(DeErr::missing_field("id"))?,
			anf: KrpcAnfrage::FindNode {
				ziel: ziel.ok_or(DeErr::missing_field("target"))?,
				will,
			},
		})
	}

	fn get_peers_argumente_einlesen<A: Addr>(
		obj: Object,
	) -> Result<KrpcInhalt<A>, DeErr> {
		let mut d = obj.try_into_dictionary()?;
		dict_einlesen!(&mut d,
		...
		Ok(KrpcInhalt::Anfrage {
			id: id.ok_or(DeErr::missing_field("id"))?,
			anf: KrpcAnfrage::GetPeers {
				info_hash: info_hash.ok_or(DeErr::missing_field("target"))?,
				will,
			},
		})
	}

	fn announce_peer_argumente_einlesen<A: Addr>(
		obj: Object,
	) -> Result<KrpcInhalt<A>, DeErr> {
		let mut d = obj.try_into_dictionary()?;
		dict_einlesen!(&mut d,
	...
	fn sample_infohashes_argumente_einlesen<A: Addr>(
		obj: Object,
	) -> Result<KrpcInhalt<A>, DeErr> {
		let mut d = obj.try_into_dictionary()?;
		dict_einlesen!(&mut d,
	...
	fn unbekannte_methode_argumente_einlesen<A: Addr>(
		obj: Object,
		name: Vec<u8>,
	) -> Result<KrpcInhalt<A>, DeErr> {
		let mut d = obj.try_into_dictionary()?;
		...
		Self: Sized,
	{
		match obj {
			Object::List(mut liste) => {
				...
				let fehlermeldung =
					String::from_utf8_lossy(fehlermeldung_obj.try_into_bytes()?)
						.into_owned();
				if liste.next_object()?.is_some() {
					return Err(länge_err());
				}
			...
			_ => Err(
				DeErr::unexpected_token("Liste mit Länge == 2", "keine Liste")
					.context("KrpcFehler"),
			),
		}
	}
		...
		Self: Sized,
	{
		Ok(U160(
			obj
				.try_into_bytes()?
				.try_into()
				.map_err(|e| DeErr::malformed_content(e).context("U160"))?,
		))
	}
}
...
fn peerliste_kodieren<'a, I: Iterator<Item = &'a SocketAddr>>(
	en: SingleItemEncoder,
	iter: I,
) -> Result<(), EnErr> {
	let mut v =
		Vec::with_capacity(SocketAddrV4::KRPC_LEN.max(SocketAddrV6::KRPC_LEN));
	en.emit_list(|e| {
		for addr in iter {
			match addr {
				SocketAddr::V4(a) => {
...
fn peerliste_dekodieren(obj: Object) -> Result<Vec<SocketAddr>, DeErr> {
	...
	while let Some(obj2) = liste_obj.next_object()? {
		...
		match peer_bytes.len() {
			6 => {
				liste_erg.push(SocketAddrV4::aus_krpc_bytes(peer_bytes).unwrap().into())
			}
			18 => {
				liste_erg.push(SocketAddrV6::aus_krpc_bytes(peer_bytes).unwrap().into())
			}
			_ => {
				return Err(DeErr::unexpected_token(
					"Bytestring mit Länge 6 doer 18 („Compact node info“)",
					format!("Bytestring mit Länge == {}", peer_bytes.len()),
				));
			}
		}
	}
...
fn hashliste_dekodieren(obj: Object) -> Result<Vec<U160>, DeErr> {
	...
	Ok(
		bytes
			.chunks(20)
			.map(|s| U160(s.try_into().unwrap()))
			.collect(),
	)
}

fn knotenliste_kodieren<
	'a,
	A: Addr + 'a,
	I: Iterator<Item = &'a KnotenInfo<A>>,
>(
	iter: I,
) -> Vec<u8> {
	let mut vec = Vec::with_capacity(iter.size_hint().0);
...
fn knotenliste_dekodieren<A: Addr>(
	obj: Object,
) -> Result<Vec<KnotenInfo<A>>, DeErr> {
	let bytes = obj.try_into_bytes()?;
	if bytes.len() % (A::KRPC_LEN + 20) != 0 {
		return Err(DeErr::unexpected_token(
			"Bytestring mit Länge % (A::KRPC_LEN + 20) == 0",
			format!("Bytestring mit Länge == {}", bytes.len()),
		));
	}
	Ok(
		bytes
			.chunks(A::KRPC_LEN + 20)
			.map(|b| {
...
impl FromBencode for Will {
		...
		Self: Sized,
	{
		...
		match (ipv4, ipv6) {
			...
			(false, false) => Err(DeErr::unexpected_token(
				"Want-Liste mit „n4“, „n6“ oder beidem",
				"Want-Liste mit weder „n4“ noch „n6“",
			)),
		}
	}
}
impl ToBencode for Will {
	...
	fn encode(&self, encoder: SingleItemEncoder) -> Result<(), EnErr> {
		encoder.emit_list(|e| {
			if *self == Will::Ipv4 || *self == Will::Beide {
				e.emit_str("n4")?;
			}
			if *self == Will::Ipv6 || *self == Will::Beide {
				e.emit_str("n6")?;
			}
			Ok(())
		})
